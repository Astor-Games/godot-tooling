...
public static class MathUtils
{
    ...
    {
        public Vector2 DampTowards(Vector2 target, double smoothing, double dt)
        {
    ...
    {
        public Vector3 Damp(Vector3 target, double smoothing, double dt)
        {
            return value.Lerp(target, GetDampWeight(smoothing, dt));
        }

        public Vector3 Sdamp(Vector3 target, double smoothing, double dt)
        {
    ...
    {
        public Basis Sdamp(Basis target, double smoothing, double dt)
        {
            return value.Slerp(target, GetDampWeight(smoothing, dt));
        }
    }
    ...
    public static float Damp(float source, float target, double smoothing, double dt)
    {
        return Mathf.Lerp(source, target, GetDampWeight(smoothing, dt));
    }

    public static float DampAngle(float source, float target, double smoothing, double dt)
    {
        return Mathf.LerpAngle(source, target, GetDampWeight(smoothing, dt));
    }

    private static float GetDampWeight(double smoothing, double dt)
    {
        return (float)(1 - Mathf.Pow(smoothing, dt));
    }
